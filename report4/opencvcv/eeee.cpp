...
int main()
{
	...
	//rightgray.convertTo(rightgray, -1, 1, 5);

	leftgray.convertTo(leftgray, -1, 1.2, 0);
	rightgray.convertTo(rightgray, -1, 1.3, 0);
	...
	//x1,x2찾기
	cv::Point savepoint[361 + 25]; //256-> 32 i-> 16으로   361(마스크 19*19)  + 25해주면!
	/*
	for (int i = 0; i < 256; i++)
	{
		//첫번째 점
...
{//375
	...
	for (int c = 0; c < leftimage.cols; c++) //마찬가지로 계산할때 c
	{
		...
		cv::Point kingpoint[361 + 25];
		for (int i = 0; i < 361 + 25; i++) /// 256개의 savepoint
		{
		...
		for (int rightGo = 0; rightGo < c; rightGo++) // leftimage.col에서 c까지로 변환 왜냐하면 어차피 왼쪽부터 검사하기 때문
		{


			cv::Point matchpoint[361 + 25];
			int temp = 0;
			for (int i1 = 0; i1 < 361 + 25; i1++)
			{
